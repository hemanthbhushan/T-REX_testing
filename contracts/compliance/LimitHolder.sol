...
contract LimitHolder is ICompliance, Ownable

 {
     ...
     */
     //here msg.sender should be the token contract other than that ut returns false
    function isToken() internal view returns (bool) {
        return isTokenBound(msg.sender); //doubt
    }
    ...
    function holderAt(uint256 index) external view returns (address) {
        require(index < shareholders.length, "shareholder doesnt exist");
     ...
     */
     //if the address has the balance it returns ntg but if the address has zero balance it will be removed from the share sholders list
    function pruneShareholders(address addr) internal {
        ...
        //mapping(address => uint256) private holderIndices;
        //inder starts from zero need to remove by 1
        uint256 holderIndex = holderIndices[addr] - 1;
         //3-1 = 2 
        uint256 lastIndex = shareholders.length - 1;
                //7-1 = 6
        address lastHolder = shareholders[lastIndex];
        //the laster is assigned to the index of the prsnt holder ,and the last holder is also assigned to the holder index of the prsnt holder
