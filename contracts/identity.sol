...
contract Identity is Storage, IIdentity, Version {
    ...
    */
    ...
    returns (bool success)
    {
        if (msg.sender != address(this)) {
            // the key is nothing but the hash of msg.sender and purpose (1= management,2 = action,3 = claim signer )
            //when the identity contract is created by default the management key is created
            //even the issuer will have the management key created by default 
            require(keyHasPurpose(keccak256(abi.encode(msg.sender)), 1), "Permissions: Sender does not have management key");
        }
        //if the user got key then it will proceed further to check if user has already existing purpose or not
  
        if (keys[_key].key == _key) {
             //here its checking weather the user already purpose exist(1= management,2= action,3 = claim signer )
            for (uint keyPurposeIndex = 0; keyPurposeIndex < keys[_key].purposes.length; keyPurposeIndex++) {
                ...
                if (purpose == _purpose) {
                    //if already the purpose exist it reverts
        ...
        //if the user dosnt have the key the _key,_purpose,_type is assigned
         else {
            keys[_key].key = _key;
    ...
    */
    ...
    returns (bool success)
    {
        ...
        //there will be four purpose for the key 1 = management 2 = action 3 = claimsigner
        ...
        //here removing the keys from the list of keysbypurpose
        uint keyIndex = 0;
    ...
    */
    ...
    returns(bool result)
    {
        ...
        //key can have different purposes 
    ...
    *
    * @param _topic The type of claim(facebook)
    * @param _scheme The scheme with which this claim SHOULD be verified or how it should be processed.(ecdsa)
    ...
    */
    function removeClaim(bytes32 _claimId) public delegatedOnly override returns (bool success) {
        if (msg.sender != address(this)) {
            //here only msg.sedner with the claim signer key can pass 3= claim signer 
